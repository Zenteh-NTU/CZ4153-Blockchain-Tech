...
const MarketDetail = () => {
    const { currentMarket, tradeTokens, setWinningBets, collectClaims, winning } = useContext(MarketTrackerContext);
    ...
    return (
        <div className="w-full flex justify-center min-h-screen bg-gradient-to-r from-blue-900 to-pink-900">
            <div className="pt-32 ">
                ...
                <h1 className="text-white mt-1">Winning Bet Result: { winning }</h1>
                <div className="pt-4">
                    <span className="inline-block bg-slate-900 rounded-full px-3 py-1 text-sm font-semibold text-white mb-2">Outcome Date: {outcomeDate(currentMarket.resultDate)}</span>
                </div>

                <div>
                    <span className="inline-block bg-slate-900 rounded-full px-3 py-1 text-sm font-semibold text-white mb-2">Market Close Date: {marketCloseDate(currentMarket.resultDate)}</span>
                </div>
                ...
                <div className=" mt-6 w-full p-5 bg-zinc-900 max-h-full rounded overflow-hidden shadow-lg">
                    ...
                    <p className=" font-mono pt-3 text-white">Price of Y-Token <span className=' text-blue-300'>(Option:{currentMarket.sides[1]})</span>: {(parseFloat(currentMarket.Y_Price).toFixed(3)).toString()} ETH each</p>
                    <p className=" font-mono pt-3 text-white">Price of N-Token <span className=' text-red-400'>(Option:{currentMarket.sides[0]})</span>: {(parseFloat(currentMarket.N_Price).toFixed(3)).toString()} ETH each</p>
                    <p className=" font-mono pt-3 text-white">Prize Pool: { (parseFloat(currentMarket.contractBalance).toFixed(3)).toString() } ETH</p>
                    <hr className='mt-4'></hr>
                    <div className="pt-4">
                        <span className="inline-block bg-slate-700 rounded-full px-3 py-1 text-sm font-semibold text-white mb-2">Market Hash: {currentMarket.contractHash}</span>
                    </div>
                    <div>
                        <span className="inline-block bg-slate-700 rounded-full px-3 py-1 text-sm font-semibold text-white mb-2">Created by: {currentMarket.ownerHash}</span>
                    </div>
                </div>
                ...
                <div className=" mt-6 w-full  bg-zinc-900 max-h-full rounded overflow-hidden shadow-lg flex flex-col items-center">
                    ...
                    <div className="m-5 p-5 lg:ml-5 lg:mr-0 lg:mt-5 lg:mb-5 bg-sky-900 max-h-full rounded overflow-hidden shadow-lg">
                        <button className="text-white" onClick={ () => winnerYTokens() }>Choose as Winner</button>
                        <h1 className="text-white bg text-2xl">Y-Token</h1>
                        <p className="pt-3 text-xl text-white">You own: { currentMarket.UserYToken } Y-Tokens</p>
                        ...
                        <div className='flexalign-middle'>
                            ...
                            <p className="pt-3 text-white">Price of Y-Token <span className=' text-blue-300'>(Option:{currentMarket.sides[1]})</span>: {(parseFloat(currentMarket.Y_Price).toFixed(3)).toString()} ETH each</p>
                    ...
                    <div className="m-5 p-5 bg-red-900 max-h-full rounded overflow-hidden shadow-lg col-start-13">
                        <button className="text-white" onClick={ () => winnerNTokens() }>Choose as Winner</button>
                        <h1 className="text-white text-2xl">N-Token</h1>
                        <p className="pt-3 text-xl text-white">You own: { currentMarket.UserNToken } N-Tokens</p>
                        <h1 className="text-xl pt-3 text-white bg sm:text-xl">I want to buy..</h1>
                        <div className='align-middle'>
                    ...
                    <div className='flex flex-col mt-12 mb-24 items-center text-center w-96'>
                        <h2 className='text-white'>Claim your gains, if any, after the winner has been anounced</h2>
                        <button className='bg-blue-500 w-24 mt-12 text-white my-8 pt-2 pb-2 pl-3 pr-3 font-mono rounded shadow-sky-700 shadow-lg' type='submit' onClick={ () => withdrawGains() }>Claim</button>
                    </div>
                </div>
            </div>
